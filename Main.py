...
#  filtrar e remover partes específicas da página.
def removeOsTags(soup):
    for data in soup(['style', 'script', 'head', 'header', 'meta', '[document]', 'title', 'footer', 'iframe', 'nav']):
        data.decompose()
        
    # expressões em uma string, separando por espaço.
...
for url in urls:
  ...
  texts = removeOsTags(soup)                         # Enviando o texto para filtrar.
  page = nlp(texts)

  # Armazenando as sentenças na lista geral.
  for sentenca in page.sents:
    textoCompleto.append(sentenca.text)

# Definição de uma variável para armazenar o vocabulário.
vocabulario = set()
for sentenca in textoCompleto:
...
colunaPalavra = 1
# armazenar a quantidade de documentos que tem tal palavra.
...
# informações dos dicionários e realizando o cálculo do TFIDF.
for i in range(1, len(bagOfWords)):
